  ...
  {$project:{
    comment: {
       '$filter': {
           input: '$comments',
           as: 'comments',
           cond: { $eq: ['$$comments._id', new ObjectID(commentID)] }
        }
     }
  }}
  ]).toArray(function(err, documents){
      console.log('BumsModel.getBum.err',err);
      console.log('BumsModel.getBum.err',documents[0].comment);
      if (documents == null) {
        return callback({
          errors:
          [
        ...
        if(documents && documents[0] && documents[0].comment && documents[0].comment[0] && documents[0].comment[0]._id == commentID){
          return callback({
            data:documents[0].comment[0].created_by
          });
        } else {
          return callback({
            errors:
...
Notification.add = function(fromData,toData,userAction, data, onID,callback){
  var Notifications = AppModel.db.collection('notifications');
  if(fromData && toData && userAction && onID && fromData._id && toData._id && fromData._id != toData._id){
    var data = {
      from:fromData,
      to:toData,
      data:data,
    ...
    if(userAction == "upvoted"){
      ...
      data.typeOfNotification = "voted";
    } else if(userAction == "commented"){
      data.typeOfNotification = "commented";
    }
    ...
    Notification.totalNotificationSentToUserOnOneTopic(fromData._id, toData._id, data.typeOfNotification, onID,function(total, returnDocuments){
      var flag = true;
      if(data.typeOfNotification == "voted" && total > 0){
        flag = false;
      }
      if(flag){
